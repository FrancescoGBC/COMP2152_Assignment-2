...
# It stores the port numbers as keys and their service names as values.
common_ports = {
    21: "FTP",
    22: "SSH",
    23: "Telnet",
...
# @property and @target.setter let you control how the attribute 
# is accessed and modified while still using simple syntax like obj.target.

# Q1: How does PortScanner reuse code from NetworkTool?
# PortScanner inherits from NetworkTool, so it can reuse its 
# properties and methods which avoids rewriting code and keeps the program simpler.
...
class PortScanner(NetworkTool):
    ...
    def scan_port(self, port):
        ...
        # Without try-except, the program could crash if there is a network error or timeout.
        # Using it allows the program to handle the error and continue scanning other ports.
        try:
    ...
    # Scanning one port at a time is slow because it waits for each port.
    # Threading lets multiple ports be scanned at the same time
    ...
    try:
        ...
        for port, status, service in results:
            ...
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?)
            ''', (target, port, status, service, current_time))
...
def load_past_scans():
    """
    Connects to the database and prints all previous scan results.
    """
    try:
        conn = sqlite3.connect("scan_history.db")
        cursor = conn.cursor()
